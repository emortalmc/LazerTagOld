 ...
 *
 * Just KStom's raycast lib but with higher fuzzyness
 * @author https://github.com/Project-Cepi/KStom/
 */
object RayCast {
    ...
    public fun castRay(
        ...
        shouldContinue: (Vector) -> Boolean = { !instance.getBlock(it.toExactBlockPosition()).isSolid },
        onBlockStep: (Vector) -> Unit = { }
    ): world.cepi.kstom.raycast.Result {
        ...
        while (step < maxDistance) {
            ...
            if (!shouldContinue.invoke(start)) {
                return world.cepi.kstom.raycast.Result(start, HitType.BLOCK, null)
            }

            // checks if there is an entity in this step -- if so, return that.
            val target = positionInEntity(instance, start.toPosition(), origin)
            if (target != null) {
                return world.cepi.kstom.raycast.Result(start, HitType.ENTITY, target)
            }
        ...
        return world.cepi.kstom.raycast.Result(start, HitType.NONE, null)
    }
    ...
    private fun minX(position: Position) = position.x - 0.25

    private fun maxX(position: Position) = position.x + 0.25

    private fun minY(position: Position) = position.y - 0.25

    private fun maxY(position: Position) = position.y + 0.25

    private fun minZ(position: Position) = position.z - 0.25

    private fun maxZ(position: Position) = position.z + 0.25
}